...
\newcommand{\assignmentduedate}{October 31}
\newcommand{\assignmentassignedate}{October 24}
...
\newcommand{\mainprogramsource}{\lstinline{src/main/java/labsix/experiment/Experiment.java}}
...
\newcommand{\testprogramsource}{\lstinline{src/test/java/labsix/TestExperiment.java}}
...
% --> Display backticks as used in Markdown code blocks
...
\newcommand{\gitcommitmainprogram}{\command{git commit src/main/java/labsix/experiment/Experiment.java -m "Your
...
experimentally evaluating a password checking program. You will learn how to
create a data table and calculate an order of growth ratio. You will also
continue to learn how to implement and test a Java program and to write a
Markdown file, practicing how to use a grading tool to assess your progress
...
GitHub and GitHub Classroom. To do well on this assignment, you should read all
of Sections 4.1 through 4.3, continuing to focus on Code Fragment 4.1 and the
commentary for Table 4.1, while also carefully studying the content about the
worst-case time complexity of an algorithm. Please see the instructor or one of
the teaching assistants if you have questions about this reading assignment.
...
``Allegheny-Computer-Science-101-Fall-2019/computer-science-101-fall-2019-lab-6-gkapfham''.
Unless you provide the instructor with documentation of the extenuating
circumstances that you are currently facing, not accepting the assignment means
...
\begin{lstlisting}
  git clone git@github.com:Allegheny-Computer-Science-101-F2019/computer-science-101-fall-2019-lab-6-gkapfham.git
\end{lstlisting}

% If you want to \step{go back} one directory from your current location, then
% you can type the command \command{cd ..}.

After this command finishes, you can use \command{cd} to change into the new
directory. Please continue to use the \command{cd} and \command{ls} commands to
explore the files that you automatically downloaded from GitHub. Each student
should draw a diagram to show the relationship between the Java classes
provided for this laboratory assignment. Your diagram should note what parts of
the Java source code from your previous assignments (e.g., the methods for
running an experiment campaign) can be reused as part of this assignment. Make
sure that you have the instructor check your diagram.

\section*{Experimentally Evaluating the Performance of Password Checking}

% Next time, consider adding more details about the code modifications
...
some pointers to help you to implement the needed code. First, you should know
that you must implement a \program{PasswordChecker} class and experimentally
evaluate it through a doubling experiment. The \program{PasswordChecker} must
enforce the rule that the input password must contain at least one of the
following: a digit, an uppercase letter, a lowercase letter, and a special
symbol (i.e., \program{@}, \program{#}, \program{\$}, or \program{\%}). The
password must contain at least six characters. Once you have finished
implementing the \program{PasswordChecker} you should also resolve all of the
\command{TODO} markers in the \program{RunCampaign} and \program{Experiment}
classes, ensuring that all of the provided tests pass. Please note that there
are many different ways to implement the \program{PasswordChecker} in Java; you
should use your creativity and expertise to implement a tool that is as
...
the \program{PasswordChecker} that you implement. For instance, when describing
the results from running an experiment, page 153 notes that ``[A]s the value of
$n$ is doubled, the running time of {\tt repeat1} typically increases more than
fourfold.'' What does this suggest about the likely ``worst-case time
complexity'' of {\tt repeat1}? How can you apply this intuition to analyze the
results that you collect when running an experiment? How can you apply what you
learned in prior experiments to this assignment's experiments? Overall, what do
the timing results and the order-of-growth ratios tell you about the worst-case
time complexity of your \program{PasswordChecker} when it checks either valid or
invalid passwords? How do you know?
...
check your program for errors and get it ready to run. Once the program runs and
the tests pass, please reflect on this process. What step did you find to be the
most challenging? Why? You should write your reflections in a file, called
\reflection{}, that uses the Markdown language. You can learn more about
Markdown by viewing the aforementioned GitHub guide. To complete this part of
the assignment, you should write one paragraph that reports on your experiences.
Your \reflection{} file should also have answers to the questions about
\mainprogram{}'s implementation, testing, and output data tables. In summary,
make sure that you can provide detailed responses to questions like these:

\vspace*{-.05in}
\begin{enumerate}
  \setlength{\itemsep}{0pt}
  \item What was your strategy for automatically checking whether a password was valid or not?
  \item What did your doubling experiment reveal about the overall performance of your checker?
  \item Using the big-Oh notation, what is the likely worst-case time complexity of your checker?
\end{enumerate}
...
the provided JUnit tests and Gradle will check to ensure that two source code
files contain the right number of \program{println} statements and see that you
commit to GitHub a sufficient number of times when completing this assignment.
This project's focus is on ensuring that the tests correctly pass.
...
checking to see that the transfer to GitHub worked.
...
\item Stored in a Markdown file called \reflection{}, a multiple-paragraph
  response, across all of the stated questions. Make sure to include the
  requested results in a fenced code block. Please see the course instructor if
  you are not sure how to use three backticks (i.e., \backticktrick{}) at the
  start and end of your code block to include a data table in your analysis.

\item A properly documented and correct version of all the Java source files
  (i.e., password checker, experiments, and tests) that meets the set
  requirements and produces the desired output.
  %
  Before the deadline for this assignment, please make sure that your source
  code is passing all of GatorGrader's checks and that each test in the provided
  test suites is also passing.
