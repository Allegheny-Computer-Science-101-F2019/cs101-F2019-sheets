...
and MacOS have suitable terminal windows available by default, you can now use
them to run package manager commands to install the \program{git} command-line
tool for your operating system. Before moving onto the next step of the
assignment, you should now have working versions of the following software
tools:
...
Before reading the installation instructions for Docker, you should know that
...
\begin{enumerate}
    ...
    \item Determine the version of your operating system. If your laptop meets
      ...
      please read the installation instructions for Docker ToolBox, provided
      later in this assignment sheet and in that program's \mbox{GitHub
      repository}.
    ...
    \item Go to \url{https://docs.docker.com/install/} and  from the menu on the
      ...
      \url{https://docs.docker.com/docker-for-windows/install/}. Linux users can
      use their package manager when following the instructions at the
      aforementioned site.
  ...
  \item Note that even though the previous Docker container caused your program
  ...
  \item You can use commands line \command{cd} and \command{ls} to explore the
    default configuration of this Ubuntu container provided by Docker. Now, type
    the commands \command{docker ps} and \command{docker images} and explain
    this output to a technical leader and a fellow student. You should also
    paste the output of these commands into the \url{#practicals} channel in
    Slack. Remember, you can paste a block of formatted text into Slack by using
    three ``backticks'', pasting the content, and then closing with three
    additional backticks. Finally, please write in Slack about what you
    experienced during the installation of Docker. What did you find confusing?
    Why did you find it to be confusing? Based on your experiences in this
    practical assignment, what do you think are the benefits associated with
    using Docker in this computer science course?
