...
\newcommand{\assignmentduedate}{December 5}
\newcommand{\assignmentassignedate}{Novembr 21}
...
\newcommand{\testprogram}{\lstinline{TestArrayList}}
...
Specifically, you will implement and test several methods of an array-based
list data structure and then run a test suite to ensure that operations, like
the \program{set}, \program{toString}, and \program{equals} methods, are
working correctly. In addition to thinking about the worst-case time complexity
of methods, you will continue to practice using Java and JUnit to implement and
test a data structure while using a grading tool to assess your progress
towards correctly completing the entire project.
...
(in Sections 7.1 and 7.2) about the \program{ArrayList} data structure. Please
see the instructor or a teaching assistant if you have questions about these
reading assignments.
...
with a name like
``Allegheny-Computer-Science-101-Fall-2019/computer-science-101-fall-2019-lab-9-gkapfham''.
...
\begin{lstlisting}
  git clone git@github.com:Allegheny-Computer-Science-101-F2019/computer-science-101-fall-2019-lab-9-gkapfham.git
\end{lstlisting}

After this command finishes, you can use \command{cd} to change into the new
directory. What files and directories do you see? What do you think is their
purpose? Specifically, each student should ensure that they fully understand
every test case in the \testprogramsource{}.

\section*{Understanding, Implementing, and Testing an \tt{ArrayList}}

There are several \command{TODO} markers inside of the provided Java source
code. Ultimately, it is your responsibility to read each of these and provide
the required Java source code. With that said, the remainder of this section
will provide some pointers to help you to implement the needed methods.
Specifically, this assignment asks you to run experiments that produce output
like that which Figure~\ref{fig:output} provides. In particular, you will
implement, test, and strive to fully understand a complete implementation of the
\program{ArrayList} data structure. Some of the missing source code in the
provided implementation can be found in Code Fragments 7.2 and 7.3 in the
textbook. Please recall that the source code in the textbook does not adhere to
the Checkstyle standards and thus you will have to improve the source code from
these Code Fragments. An important method that you must implement in the
\program{ArrayList} is the \program{toString} that produces a textual
representation of the array-based data structure. If the \program{ArrayList}
contains a total of $n$ elements then the \program{toString} method should
return a \program{String} of the form ``$(e_1, e_2, \ldots, e_n)$''. Once you
...
the questions about the \program{ArrayList}'s implementation, testing, and use
of Java language and JUnit features (e.g., \program{@Test(expected =
...
the provided JUnit tests and GatorGrader will check to ensure that a source code
files contains the right number of \program{println} statements and see that you
commit to GitHub a sufficient number of times when completing this assignment.
The focus of this assignment is checking to ensure that the tests pass and do
not lead to either \program{java.lang.Exception} or
\program{java.lang.AssertionError}.
...
% percentage of checks that are currently passing.
...
building correctly, fulfilling at least some of the laboratory assignment's
...
typing \gitpush{} and checking to see that the transfer to GitHub worked
correctly.
...
\item Stored in a Markdown file called \reflection{}, a one-paragraph response
  for each of the stated questions. Make sure to fully explain and justify your
...
still considering the regularity with which you committed to your repository and
the overall quality of your partially completed work.

% Had to cut this content due to space constraints

% Please see the instructor if you have questions about the evaluation of this
