...
\item {\bf Explore teamwork and technologies}. While certain aspects of the
  ...
  regularly communicate with your team and/or the technical leaders and tutors.
...
Chapter 1 in the textbook, paying particularly close attention to Sections 1.1
through 1.3 and Section 1.8. Please see the instructor or one of the technical
leaders if you have questions on these sections.
...
\begin{lstlisting}
  git clone git@github.com:Allegheny-Computer-Science-101-F2019/computer-science-101-fall-2019-practical-1-gkapfham.git
\end{lstlisting}
...
a \command{docker run} command to start \command{gradle grade} as a
containerized application, using the ``DockaGator'' Docker image available on
DockerHub.
%
Before you start to build and test your program, please study the source code of
the \mainprogram~program, noticing that one of its methods is incomplete.
Specifically, you will notice that the program is missing an implementation of
the \program{swap} method. Can you add in a full implementation of this method?
Why do you think that the \program{swap} method accepts parameters of type
\program{AtomicInteger}?
...
passes even though the program is incorrect. Why? Finally, you should add at
least two new test cases to this suite. What type of tests are most needed?
...
Do you see that it produces correct output like in Figure~\ref{fig:output}? If
not, then determine what is wrong and then repair the program and re-build and
re-run it.

\section*{Checking the Correctness of Your Program and Writing}

As verified by Checkstyle tool, the source code for the \mainprogram{} and
...
instance, Gradle will run the provided JUnit tests, check to ensure that
\mainprogram{} produces exactly six lines of output, and GatorGrader will see
if you committed to GitHub enough times when completing this practical
assignment.
...
terminal, followed by typing \gitpush{} to perform the transfer.
...
When you use the \gitpush{} command to transfer your source code to your GitHub
repository, Travis CI will initialize a \step{build} of your assignment,
checking to see if it meets all of the requirements. If both your program and
test suite meet all of the established requirements, then you will see a green
...
Please note that when you complete this practical assignment on your own laptop
you will need to ensure that you run the correct Docker commands for your
specific operating system. You can look in the \program{README.md} file in your
GitHub repository for a listing of the Docker commands that you can use during
the completion of this practical assignment. Please bear in mind that,
especially if you are using Windows 10 Pro or Windows 10 Enterprise, you may
need to customize some of these commands so that they work correctly for your
laptop.
%
With that said, don't forget that assignment checking will still work with
Travis CI even if you do not have the programs like Docker installed on your
laptop. Please see a technical leader if you cannot install these programs.
